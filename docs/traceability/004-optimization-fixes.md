...
| FR-012 | Internal performance change: symmetric session keys are prepared once per secure channel and reused per MSG chunk. | N/A - internal crypto optimization; encoded secure-channel message behaviour must stay unchanged. | (none) | DEFERRED - no implementation retained; initial global cipher-context registry was reverted because dangling pointers across channel reuse could crash. Clean follow-up should thread per-channel context through the `sym_chunk` AES calls; the per-message stateless AES path is unchanged/correct. | DEFERRED - no retained implementation to test. | DEFERRED |
