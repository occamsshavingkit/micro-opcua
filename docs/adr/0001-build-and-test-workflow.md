...
The `micro-opcua` project requires a predictable workflow for host development, embedded cross-compilation, and testing. A standard developer build target summary ensures all developers use reproducible tooling. 
...
1. **Host Build and Test**: `cmake -B build/host -DMICRO_OPCUA_BUILD_TESTS=ON` and `ctest`.
2. **Formatting and Static Analysis**: Targets `format-check`, `cppcheck`, and `clang-tidy` generated in the host build.
3. **Sanitizers**: `build/asan` with `-DMICRO_OPCUA_SANITIZERS="address,undefined"`.
4. **Fuzzing**: `build/fuzz` with `-DMICRO_OPCUA_BUILD_FUZZERS=ON`.
5. **Cross-Compilation (Pico)**: `build/pico` with `-DMICRO_OPCUA_PLATFORM=pico`.
