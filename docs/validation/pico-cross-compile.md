...
- Local configure command:
  `cmake -S . -B build/us4-pico -DMICRO_OPCUA_PLATFORM=pico -DPICO_SDK_FETCH_FROM_GIT=ON -DMICRO_OPCUA_EMBEDDED_PROFILE=ON -DMU_MAX_SUBSCRIPTIONS=2 -DMU_MAX_MONITORED_ITEMS=100 -DMU_MAX_PUBLISH_REQUESTS=5 -DMU_MONITORED_QUEUE_DEPTH=2 -DMU_MAX_TRIGGER_LINKS=4 -DMICRO_OPCUA_BUILD_EXAMPLES=ON`
- Local build command:
  `cmake --build build/us4-pico`
- Result: passed. The Pico SDK configured for `PICO_PLATFORM=rp2040`,
  `PICO_BOARD=pico`, and `PICO_GCC_TRIPLE=arm-none-eabi`; `micro_opcua`,
  `micro_opcua_pico`, and `pico_minimal_server` built.
- Build output:
  - `build/us4-pico/src/libmicro_opcua.a`
