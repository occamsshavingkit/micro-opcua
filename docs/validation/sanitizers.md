...
- Configure command:
  ...
  - Enabled knobs: `MUC_OPCUA_BUILD_TESTS=ON`,
...
- Build command: `cmake --build build-san`
  - Result: PASS, exit code `0`.
  - Produced `build-san/src/libmuc_opcua.a` and sanitizer-linked test
...
- Note: the repository's `MUC_OPCUA_SANITIZERS` cache variable is not currently wired to
...
- Configure/build:
  `cmake -S . -B build/us4-ubsan-asan -DMUC_OPCUA_BUILD_TESTS=ON -DMUC_OPCUA_SUBSCRIPTIONS_STANDARD=ON -DMUC_OPCUA_BASE_TYPE_SYSTEM=ON -DMU_MAX_MONITORED_ITEMS=100 -DMU_MAX_SUBSCRIPTIONS=2 -DMU_MAX_PUBLISH_REQUESTS=5 -DMU_MONITORED_QUEUE_DEPTH=2 -DMU_MAX_TRIGGER_LINKS=4 -DCMAKE_C_FLAGS='-fsanitize=address,undefined -fno-omit-frame-pointer' -DCMAKE_EXE_LINKER_FLAGS='-fsanitize=address,undefined'`
