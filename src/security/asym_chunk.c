...
static opcua_statuscode_t write_header(mu_binary_writer_t *w, mu_security_policy_id_t policy,
                                       opcua_uint32_t scid,
    ...
    if (!uri) return MU_STATUS_BAD_SECURITYPOLICYREJECTED;

    w->buffer[0] = 'O'; w->buffer[1] = 'P'; w->buffer[2] = 'N'; w->buffer[3] = 'F';
    w->position = 4;
...
static opcua_statuscode_t wrap_none(opcua_uint32_t scid, opcua_uint32_t seq, opcua_uint32_t rid,
    ...
    if (s != MU_STATUS_GOOD) return s;
    s = mu_binary_write_uint32(&w, seq);   if (s != MU_STATUS_GOOD) return s;
    s = mu_binary_write_uint32(&w, rid);   if (s != MU_STATUS_GOOD) return s;
    if (w.position + body_len > out_cap) return MU_STATUS_BAD_RESPONSETOOLARGE;
    memcpy(out + w.position, body, body_len);
    size_t total = w.position + body_len;
    /* Patch MessageSize. */
    out[4] = (opcua_byte_t)(total); out[5] = (opcua_byte_t)(total >> 8);
    out[6] = (opcua_byte_t)(total >> 16); out[7] = (opcua_byte_t)(total >> 24);
    *out_len = total;
    return MU_STATUS_GOOD;
}
...
opcua_statuscode_t mu_asym_chunk_wrap(
    ...
    opcua_byte_t *out, size_t out_cap, size_t *out_len)
{
    if (!out || !out_len || (!body && body_len)) return MU_STATUS_BAD_INTERNALERROR;
    ...
    const opcua_byte_t *sender_cert = NULL; size_t sender_cert_len = 0;
    ...
    s = key_bytes(crypto, sender_cert, sender_cert_len, &sig_len);     if (s != MU_STATUS_GOOD) return s;
    s = key_bytes(crypto, receiver_cert, receiver_cert_len, &recv_key_bytes); if (s != MU_STATUS_GOOD) return s;
    ...
    mu_asym_chunk_info_t *info)
{
    ...
    if (msg_size > chunk_len) return MU_STATUS_BAD_DECODINGERROR;
    ...
    s = mu_binary_read_string(&r, &policy_uri);        if (s != MU_STATUS_GOOD) return s;
    s = mu_binary_read_bytestring(&r, &sender_cert);   if (s != MU_STATUS_GOOD) return s;
    s = mu_binary_read_bytestring(&r, &receiver_thumb);if (s != MU_STATUS_GOOD) return s;
    ...
    if (info->policy == MU_SECURITY_POLICY_NONE_ID) {
        ...
        if (hdr_len + 8 > msg_size) return MU_STATUS_BAD_DECODINGERROR;
    ...
    const opcua_byte_t *own_cert = NULL; size_t own_cert_len = 0;
    ...
    s = key_bytes(crypto, own_cert, own_cert_len, &own_key_bytes);            if (s != MU_STATUS_GOOD) return s;
    s = key_bytes(crypto, info->sender_cert, info->sender_cert_len, &sig_len); if (s != MU_STATUS_GOOD) return s;
    ...
    info->request_id = (opcua_uint32_t)plain[4] | ((opcua_uint32_t)plain[5] << 8) |
