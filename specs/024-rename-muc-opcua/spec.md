...
**Independent Test**: Grep the full documentation set (README, CLAUDE.md, AGENTS.md, ROADMAP.md, everything under `docs/`) for the literal strings `micro-opcua`, `micro_opcua`, and `MICRO_OPCUA` and confirm zero matches outside of files explicitly documenting historical spec content (specs/001 through specs/023, which are intentionally left as history except for updates needed so they don't misdescribe the current build).
...
2. **Given** any file under `docs/`, **When** it references the repository URL or library/macro names, **Then** it points at `github.com/occamsshavingkit/muc-opcua` and uses the new identifier names.
3. **Given** `specs/001` through `specs/023`, **When** a maintainer reads them after the rename, **Then** the historical narrative is unchanged, but any literal build-system reference (macro name, include path, repo URL) that a reader might copy-paste into a current build is corrected so it actually works.
...
- How does the project handle the case where `specs/001` through `specs/023` contain code snippets or exact file paths that literally quote old macro names as part of describing what a past PR changed — these are historical record and must not be rewritten to claim the past PR did something it didn't, only literal build-system references that a reader might copy into a *current* build need correcting.
...
- **FR-006**: Historical spec files `specs/001-*` through `specs/023-*` MUST NOT have their narrative content rewritten, but any literal reference to a macro name, include path, or repository URL that a reader could copy into a *current* build or clone MUST be corrected so it doesn't silently fail against the renamed project.
...
- **FR-008**: The project MUST add or extend an automated check (mirroring the existing stale-claim doc tests) that fails the test suite if a non-historical, non-changelog file reintroduces `micro-opcua`, `micro_opcua`, or `MICRO_OPCUA_` as a literal string.
...
- **Documentation corpus**: README, CLAUDE.md, AGENTS.md, ROADMAP.md, `docs/**` — living documents updated in place; `specs/001-023` treated as history and only corrected where a literal reference would actively mislead a reader building the current project.
...
- **SC-006**: An automated check fails the build/test suite if any non-historical, non-changelog file reintroduces the old name strings, so the rename cannot silently regress in a later PR.
...
- `specs/001` through `specs/023` are historical record of already-shipped, already-merged features; this project corrects only literal build-system references within them (macro names, include paths, repo URLs) and does not otherwise rewrite their content.
