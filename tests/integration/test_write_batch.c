/* tests/integration/test_write_batch.c
 * End-to-end integration test for batch Write requests containing multiple WriteValue items.
 * Citing OPC-10000-4 §5.11.4.2. */
...
void test_integration_write_batch(void) {
    mock_t mock;
    memset(&mock, 0, sizeof(mock));

    /* Setup address space: MyVar1 (ns=1, numeric=1000) and MyVar2 (ns=1, numeric=2000) */
    static const mu_value_source_t var_value1 = {MU_VALUESOURCE_STATIC, {.static_value = {MU_TYPE_INT32, {.i32 = 42}}}};
    static const mu_value_source_t var_value2 = {MU_VALUESOURCE_STATIC, {.static_value = {MU_TYPE_FLOAT, {.f = 1.0f}}}};
    ...
    /* Batch Write MyVar1 (Int32) and MyVar2 (Float) */
    mu_binary_writer_init(&w, tmp, sizeof(tmp));
    {
    ...
    mu_binary_write_int32(&w, 2); // 2 write values
    ...
    dv0.value.value.i32 = 123;
    mu_binary_write_datavalue(&w, &dv0);
    ...
    dv1.value.type = MU_TYPE_FLOAT;
    dv1.value.is_array = false;
    dv1.value.value.f = 2.5f;
    mu_binary_write_datavalue(&w, &dv1);
    ...
    g_batch_write_count = 0;
    mu_server_poll(server); /* Write -> Response */

    TEST_ASSERT_EQUAL(2, g_batch_write_count);
    ...
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mu_binary_read_statuscode(&body, &item_status0));
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, item_status0);

    opcua_statuscode_t item_status1;
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mu_binary_read_statuscode(&body, &item_status1));
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, item_status1);
}
#endif
...
int main(void) {
    UNITY_BEGIN();
#ifdef MICRO_OPCUA_SERVICE_WRITE
    RUN_TEST(test_integration_write_batch);
#endif
    return UNITY_END();
}
