...
static opcua_statuscode_t m_listen(void *c, const char *u) { (void)c; (void)u; return MU_STATUS_GOOD; }
static void m_shutdown(void *c) { (void)c; }
static void m_close(void *c, void *h) { (void)c; (void)h; }
static opcua_statuscode_t m_accept(void *c, void **h) {
    mock_t *m = c; m->accept_count++;
...
static size_t build_hello(opcua_byte_t *out) {
    mu_binary_writer_t w; mu_binary_writer_init(&w, out, 256);
    out[0]='H'; out[1]='E'; out[2]='L'; out[3]='F'; w.position = 4;
    ...
    mu_binary_writer_t hs; mu_binary_writer_init(&hs, out, 256); hs.position = 4; mu_binary_write_uint32(&hs, (opcua_uint32_t)w.position);
    return w.position;
}

static size_t build_opn(opcua_byte_t *out) {
    mu_binary_writer_t w; mu_binary_writer_init(&w, out, 256);
    out[0]='O'; out[1]='P'; out[2]='N'; out[3]='F'; w.position = 4;
    mu_binary_write_uint32(&w, 0);
    mu_binary_write_uint32(&w, 0);
...
static void configure(mu_server_config_t *config, mock_t *mock, opcua_byte_t *rx, opcua_byte_t *tx) {
    ...
    config->application_uri = "u"; config->product_uri = "u"; config->application_name = "t";
    config->receive_buffer = rx; config->receive_buffer_size = 8192;
    config->send_buffer = tx; config->send_buffer_size = 8192;
    ...
    config->tcp_adapter.listen = m_listen; config->tcp_adapter.accept = m_accept;
    config->tcp_adapter.read = m_read; config->tcp_adapter.write = m_write;
    config->tcp_adapter.close_connection = m_close; config->tcp_adapter.shutdown = m_shutdown;
}
...
void test_coalesced_messages_both_processed(void) {
    ...
    mu_server_config_t config; opcua_byte_t rx[8192], tx[8192];
