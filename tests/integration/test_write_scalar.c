/* tests/integration/test_write_scalar.c
 * End-to-end integration test: drives a client Write request (Int32) through HEL -> OPN ->
 * CreateSession -> ActivateSession -> Write with a mock transport.
 * Citing OPC-10000-4 §5.11.4. */
...
void test_integration_write_happy_path(void) {
    ...
    /* Setup address space: MyVar1 (ns=1, numeric=1000) */
    ...
    /* Write MyVar1 (ns=1, identifier=1000) Value = 12345 */
    mu_binary_writer_init(&w, tmp, sizeof(tmp));
    {
    ...
    mu_binary_write_int32(&w, 1); // array size
    mu_nodeid_t target_var = {1, MU_NODEID_NUMERIC, {1000}};
    mu_binary_write_nodeid(&w, &target_var);
    mu_binary_write_int32(&w, 13); // attributeId_Value
