...
static opcua_uint64_t mock_tick(void *c) {
    (void)c;
    return g_tick_ms;
}
static opcua_datetime_t mock_time(void *c) {
    (void)c;
    return 0;
}
...
static opcua_statuscode_t mock_read(void *c, void *h, opcua_byte_t *buf, size_t cap, size_t *n) {
    mock_t *m = (mock_t *)c;
    (void)h;
    if (m->next_read_len == 0 || m->next_read_len > cap) {
        *n = 0;
        return MU_STATUS_GOOD;
    }
...
static void make_config(mu_server_config_t *config, mock_t *mock, opcua_byte_t *rx, size_t rxn, opcua_byte_t *tx,
    ...
    config->receive_buffer_size = rxn;
    config->send_buffer = tx;
    config->send_buffer_size = txn;
    config->max_chunk_count = 1;
    config->max_message_size = (opcua_uint32_t)txn;
    ...
    config->time_adapter.get_time = mock_time;
    config->time_adapter.get_tick_ms = mock_tick;
...
static size_t build_hello(opcua_byte_t *buf, size_t cap) {
    ...
    }
    {
        ...
        mu_binary_writer_init(&hs, buf, cap);
        hs.position = 4;
...
void test_activity_resets_idle_timer(void) {
    ...
    opcua_byte_t hello[128];
    size_t hlen = build_hello(hello, sizeof(hello));
    g_tick_ms = 20000;
    mock.next_read = hello;
    mock.next_read_len = hlen;
