...
static opcua_statuscode_t mock_accept(void *c, void **handle) {
    mock_t *m = (mock_t *)c; m->accept_count++;
...
static opcua_statuscode_t mock_write(void *c, void *h, const opcua_byte_t *buf, size_t len, size_t *n) {
    mock_t *m = (mock_t *)c; (void)h;
    memcpy(m->last_write, buf, len); m->last_write_len = len; *n = len;
    return MU_STATUS_GOOD;
}
...
static void secure_call(mock_t *mock, mu_server_t *server, mu_crypto_adapter_t *cc,
    ...
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD,
        mu_sym_chunk_wrap(cc, MU_MESSAGE_SECURITY_MODE_SIGN_AND_ENCRYPT, c2s, "MSG",
                          scid, token, seq, seq, body, body_len, chunk, sizeof(chunk), &mlen));
...
static void run_handshake_for_policy(mu_security_policy_id_t policy_id) {
    ...
    static const mu_value_source_t var_value = {
    ...
    static const mu_node_t nodes[] = {
        { { 0, MU_NODEID_NUMERIC, { 85 } }, MU_NODECLASS_OBJECT,
          { 7, (const opcua_byte_t *)"Objects" }, { 7, (const opcua_byte_t *)"Objects" }, NULL, 0, NULL },
        { { 1, MU_NODEID_NUMERIC, { 1000 } }, MU_NODECLASS_VARIABLE,
          { 6, (const opcua_byte_t *)"MyVar1" }, { 6, (const opcua_byte_t *)"MyVar1" }, var_refs, 1, &var_value }
    };
    static const mu_address_space_t space = { nodes, 2 };

    opcua_byte_t tmp[1024], chunk[CHUNK_CAP]; mu_binary_writer_t w; size_t clen;
    opcua_byte_t client_nonce[32];
    for (int i = 0; i < 32; i++) client_nonce[i] = (opcua_byte_t)(i + 1);
    ...
    opcua_uint32_t spv, scid, token_id, revised; opcua_int64_t created;
    ...
    { mu_string_t ns = {-1, NULL}; mu_bytestring_t nb = {-1, NULL};
      mu_binary_write_string(&w, &ns); mu_binary_write_bytestring(&w, &nb);
...
int main(void) { UNITY_BEGIN(); RUN_TEST(test_secure_handshake_skipped); return UNITY_END(); }
#endif
